...
def main() -> int:
    ...
    for rel, count in observed_includes.items():
        ...
        if count != exp:
            errors.append(f"Arduino include count mismatch in {rel}: observed={count}, expected={exp}")

    for rel, exp in ALLOWED_INCLUDE_COUNTS.items():
        obs = observed_includes.get(rel, 0)
        if obs != exp:
            errors.append(f"Arduino include count mismatch in {rel}: observed={obs}, expected={exp}")
