...
Compact notes in `docs/extracted-md/` summarize LSM6DS3TR-C register, bus,
timing, FIFO, interrupt, and scale facts. Mechanically extracted text for the
primary LSM6DS3TR-C sources lives in `docs/pdf-extracted-md/`.

The compact notes are a curated index, not a replacement for the PDF tables.
They keep stable anchors for library-relevant registers and features so edits
do not lose important datasheet coverage;
`tools/check_chip_docs_coverage.py` enforces those anchors in CI.
...
Optional general MEMS design tips remain as PDFs under `docs/design_tips/`.
Generated raw text and derived summaries are not retained because these sources
do not define LSM6DS3TR-C registers or the public library contract.
...
| `08_variant_and_source_notes.md` | Naming/revision notes and supplemental document boundaries. |

## Scope Notes

- The primary datasheet is authoritative for register addresses and bit
  meanings in these compact notes.
- Design-tip PDFs are not used for LSM6DS3TR-C register addresses, reset
  values, or bit meanings unless a compact note names the specific source.
